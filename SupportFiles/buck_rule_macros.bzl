load("//SupportFiles:configs.bzl", "test_configs", "library_configs", "pod_library_configs", "DEVELOPMENT_LANGUAGE")

def root_buck():
    return native.read_config('custom', 'root_buck')
...
def apple_test_lib(
    ...
    if info_plist == None:
        info_plist = "//SupportFiles:TestInfoPlist"

    substitutions = {
        "CURRENT_PROJECT_VERSION": "1",
        "DEVELOPMENT_LANGUAGE": DEVELOPMENT_LANGUAGE,
        "EXECUTABLE_NAME": name,
        "PRODUCT_BUNDLE_IDENTIFIER": "br.com.fernandodelrio.%s" % name,
        "PRODUCT_NAME": name,
    }
